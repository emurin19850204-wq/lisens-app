...
export default function EditStaffClient({
  ...
  const staffUser = getUserById(id);

  const [name, setName] = useState(staffUser?.name || '');
  const [email, setEmail] = useState(staffUser?.email || '');
  const [role, setRole] = useState<UserRole | ''>(staffUser?.role || '');
  const [organizationId, setOrganizationId] = useState(staffUser?.organizationId || '');
  const [hireDate, setHireDate] = useState(staffUser?.hireDate || '');
    ...
    return (
      <div className="page-container">
        <div className="empty-state">
          <div className="empty-state-icon">❌</div>
          <p>スタッフが見つかりません</p>
          <Link href="/admin/staff" className="btn btn-outline" style={{ marginTop: 'var(--space-md)' }}>← スタッフ一覧に戻る</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-container">
      <nav className="breadcrumb">
        ...
        <span className="breadcrumb-separator">/</span>
        <span>{staffUser.name} を編集</span>
      </nav>

      <h1 className="page-title">✏️ スタッフ編集</h1>
      <p className="text-secondary" style={{ marginBottom: 'var(--space-lg)' }}>
        <strong>{staffUser.name}</strong> さんの情報を編集します。
      </p>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="card" style={{ marginBottom: 'var(--space-md)' }}>
          ...
          <div className="card-body">
            {id === currentUser.id && (
              <div style={{ padding: 'var(--space-sm) var(--space-md)', borderRadius: 'var(--border-radius-sm)', background: 'var(--color-warning-bg, #fff8e1)', border: '1px solid var(--color-warning)', marginBottom: 'var(--space-md)' }}>
                <span className="text-sm">⚠️ 自分自身のロールを変更すると、ログアウトが必要になる場合があります。</span>
              </div>
            )}
