...
export default function NewStaffPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await addStaff({ name: name.trim(), email: email.trim(), role, organizationId, hireDate: hireDate || null });
    router.push('/admin/staff');
  };

  return (
    <div className="page-container">
      <nav className="breadcrumb"><Link href="/">ホーム</Link><span className="breadcrumb-separator">/</span><Link href="/admin/staff">スタッフ管理</Link><span className="breadcrumb-separator">/</span><span>新規登録</span></nav>
      <h1 className="page-title">🛡️ スタッフ 新規登録</h1>
      <p className="text-secondary" style={{ marginBottom: 'var(--space-lg)' }}>管理者・教育責任者・評価者・店舗責任者を新規登録します。</p>
      <form onSubmit={handleSubmit}>
        ...
        <div className="card" style={{ marginBottom: 'var(--space-md)' }}><div className="card-header">🛡️ ロール（権限レベル）</div><div className="card-body">
          <p className="text-sm text-secondary" style={{ marginBottom: 'var(--space-md)' }}>登録するスタッフの権限レベルを選択してください。</p>
        ...
        <div className="card" style={{ marginBottom: 'var(--space-lg)' }}><div className="card-header">👁️ 登録プレビュー</div><div className="card-body"><table><tbody>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-md)', justifyContent: 'flex-end' }}>
          ...
          <button type="submit" className="btn btn-primary" disabled={isSubmitting}>{isSubmitting ? '登録中...' : '🛡️ スタッフを登録する'}</button>
        </div>
      </form>
    </div>
  );
