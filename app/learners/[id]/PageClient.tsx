/**
 * LISENS - 研修者詳細画面（個人カルテ）（Supabase対応）
 */
'use client';
...
export default function LearnerDetailClient({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (currentUser.role === 'learner' && currentUser.id !== id) {
    return (
      <div className="page-container"><div className="empty-state"><div className="empty-state-icon">🔒</div><p>他の研修者のカルテは閲覧できません</p>
        <Link href={`/learners/${currentUser.id}`} className="btn btn-outline" style={{ marginTop: 'var(--space-md)' }}>← マイカルテに戻る</Link>
      </div></div>
    );
  }
  ...
  if (!detail) {
    return (
      <div className="page-container"><div className="empty-state"><div className="empty-state-icon">❌</div><p>研修者が見つかりません</p>
        <Link href={currentUser.role === 'learner' ? '/' : '/learners'} className="btn btn-outline" style={{ marginTop: 'var(--space-md)' }}>← {currentUser.role === 'learner' ? 'ホームに戻る' : '研修者一覧に戻る'}</Link>
      </div></div>
    );
  }
  ...
  const handleDateChange = async (subjectId: string, field: 'startedAt' | 'completedAt', value: string) => {
    let currentStarted: string | null = null;
    let currentCompleted: string | null = null;
    for (const cp of curriculumProgresses) {
      const sp = cp.subjects.find(s => s.subject.id === subjectId);
      if (sp) { currentStarted = sp.progress?.startedAt || null; currentCompleted = sp.progress?.completedAt || null; break; }
    }
  ...
  const handleHoursChange = async (subjectId: string, hours: number) => {
    if (hours > 0) { await updateSubjectHours(subjectId, hours); await refresh(); }
  };
  ...
  return (
    <div className="page-container">
      <nav className="breadcrumb">
        <Link href="/">ホーム</Link><span className="breadcrumb-separator">/</span>
        <Link href="/learners">研修者一覧</Link><span className="breadcrumb-separator">/</span>
      ...
      <div className="card" style={{ marginBottom: 'var(--space-lg)' }}>
        <div className="card-body">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 'var(--space-md)' }}>
            <div>
              <h1 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: 'var(--space-xs)' }}>{learner.name}</h1>
              ...
              <div style={{ marginTop: 'var(--space-sm)', display: 'flex', gap: 'var(--space-sm)', flexWrap: 'wrap', alignItems: 'center' }}>
                <span className={`badge ${LEVEL_BADGE_CLASS[learner.currentLevel]}`} style={{ fontSize: '0.85rem', padding: '4px 12px' }}>🏆 {currentLevel.name}</span>
                {learner.tracks.length > 0 && learner.tracks.map(t => (
                  <span key={t} className={`badge ${TRACK_BADGE_CLASS[t]}`} style={{ fontSize: '0.8rem' }}>{TRACK_LABELS[t]}</span>
                ))}
              </div>
            </div>
            <div className="flex gap-sm" style={{ flexWrap: 'wrap' }}>
              {currentUser.role !== 'learner' && <Link href="/learners" className="btn btn-outline">← 一覧に戻る</Link>}
              {canEvaluate && <Link href={`/evaluations/new?learner_id=${learner.id}`} className="btn btn-primary">📝 OSCE評価</Link>}
              {canApplyCert && <Link href={`/certifications/new?learner_id=${learner.id}`} className="btn btn-outline">🏆 認定申請</Link>}
            </div>
          </div>
        </div>
      </div>
        ...
        <div>
          {canEditProgress && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 'var(--space-md)', gap: 'var(--space-sm)' }}>
              <button className={`btn ${isEditingProgress ? 'btn-primary' : 'btn-outline'}`} onClick={() => setIsEditingProgress(!isEditingProgress)}>
          ...
          {curriculumProgresses.map(cp => (
            <div key={cp.curriculum.id} className="card" style={{ marginBottom: 'var(--space-md)' }}>
              <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div><span>{cp.curriculum.name}</span><span className="text-sm text-secondary" style={{ marginLeft: 'var(--space-sm)' }}>({cp.curriculum.totalHours}h)</span></div>
              ...
              <div className="card-body" style={{ padding: 0 }}>
                <table>
                  <thead><tr><th>コマ</th><th>時間</th><th>ステータス</th><th>開始日</th><th>完了日</th><th style={{ width: '40px', textAlign: 'center' }}>📝</th></tr></thead>
                  <tbody>
                    {cp.subjects.map(sp => {
                      ...
                      return (
                        <>
                          <tr key={subjectId} style={{ cursor: 'pointer' }} onClick={() => toggleSubjectExpand(subjectId)}>
                            <td><span style={{ marginRight: 'var(--space-sm)' }}>{sp.progress?.status === 'completed' ? '☑' : '☐'}</span>{sp.subject.name}</td>
                            <td className="text-sm text-secondary">
                              {isEditingProgress && canEditProgress ? (
                                <input type="number" className="form-input" style={{ padding: '2px 6px', fontSize: '0.8rem', width: '60px', textAlign: 'center' }} value={sp.subject.hours} min={0.5} step={0.5} onClick={e => e.stopPropagation()} onChange={e => handleHoursChange(subjectId, parseFloat(e.target.value))} />
                              ) : (<>{sp.subject.hours}h</>)}
                            </td>
                            <td>
                              {isEditingProgress && canEditProgress ? (
                                <select className="form-input" style={{ padding: '2px 6px', fontSize: '0.8rem', width: 'auto', minWidth: '80px' }} value={sp.progress?.status || 'not_started'} onClick={e => e.stopPropagation()} onChange={e => handleStatusChange(subjectId, e.target.value as ProgressStatus)}>
                                  <option value="not_started">未着手</option><option value="in_progress">受講中</option><option value="completed">完了</option>
                                </select>
                              ) : (
                                <span className={`badge ${PROGRESS_STATUS_BADGE_CLASS[sp.progress?.status || 'not_started']}`}>{PROGRESS_STATUS_LABELS[sp.progress?.status || 'not_started']}</span>
                              )}
                            </td>
                            <td className="text-sm text-secondary">
                              {isEditingProgress && canEditProgress ? (
                                <input type="date" className="form-input" style={{ padding: '2px 4px', fontSize: '0.75rem', width: 'auto', minWidth: '120px' }} value={sp.progress?.startedAt ? sp.progress.startedAt.slice(0, 10) : ''} onClick={e => e.stopPropagation()} onChange={e => handleDateChange(subjectId, 'startedAt', e.target.value)} />
                              ) : (<>{sp.progress?.startedAt ? new Date(sp.progress.startedAt).toLocaleDateString('ja-JP') : '—'}</>)}
                            </td>
                            <td className="text-sm text-secondary">
                              {isEditingProgress && canEditProgress ? (
                                <input type="date" className="form-input" style={{ padding: '2px 4px', fontSize: '0.75rem', width: 'auto', minWidth: '120px' }} value={sp.progress?.completedAt ? sp.progress.completedAt.slice(0, 10) : ''} onClick={e => e.stopPropagation()} onChange={e => handleDateChange(subjectId, 'completedAt', e.target.value)} />
                              ) : (<>{sp.progress?.completedAt ? new Date(sp.progress.completedAt).toLocaleDateString('ja-JP') : '—'}</>)}
                            </td>
                            <td style={{ textAlign: 'center' }}>
                              {hasMemo && <span style={{ fontSize: '0.9rem' }}>{isLearner ? '💬' : '📝'}</span>}
                              <span style={{ fontSize: '0.7rem', marginLeft: '2px' }}>{isExpanded ? '▲' : '▼'}</span>
                            </td>
                          </tr>
                          {isExpanded && (
                            <tr key={`${subjectId}-memo`}><td colSpan={6} style={{ padding: 0, border: 'none' }}>
                              <div style={{ padding: 'var(--space-sm) var(--space-md)', background: 'var(--color-bg)', borderTop: '1px dashed var(--color-border)', borderBottom: '1px dashed var(--color-border)' }}>
                                ...
                                {canEditProgress && isEditingMemo(subjectId) ? (
                                  <div>
                                    ...
                                    <div style={{ display: 'flex', gap: 'var(--space-sm)', marginTop: 'var(--space-xs)', justifyContent: 'flex-end' }}>
                                      <button className="btn btn-outline btn-sm" onClick={() => setEditingMemos(prev => { const n = { ...prev }; delete n[subjectId]; return n; })}>キャンセル</button>
                                      <button className="btn btn-primary btn-sm" onClick={() => handleMemoSave(subjectId)}>💾 保存</button>
                                    </div>
                                  </div>
                                ) : (
                                  <div>
                                    {currentMemo ? (
                                      <div style={{ padding: 'var(--space-sm)', background: 'var(--color-surface)', borderRadius: 'var(--border-radius-sm)', borderLeft: '3px solid var(--color-primary)', fontSize: '0.85rem', lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>{currentMemo}</div>
                                    ) : (
                                      <div className="text-sm text-secondary" style={{ fontStyle: 'italic', padding: 'var(--space-xs) 0' }}>{isLearner ? 'まだ講師からのコメントはありません' : 'メモはありません'}</div>
                                    )}
                                    {canEditProgress && (
                                      <button className="btn btn-outline btn-sm" style={{ marginTop: 'var(--space-xs)' }} onClick={() => startEditMemo(subjectId, currentMemo)}>✏️ {currentMemo ? 'メモを編集' : 'メモを追加'}</button>
                                    )}
                                  </div>
                                )}
                              </div>
          ...
          {evaluations.length === 0 ? (
            ...
            evaluations.map(ev => {
              ...
              (['C', 'E', 'P', 'T', 'S', 'EN'] as EvaluationSectionCode[]).forEach(sec => {
                const secItems = ev.items.filter(i => { const tmpl = EVALUATION_TEMPLATES.find(t => t.name === i.itemName); return tmpl?.section === sec; });
                sectionScores[sec] = { score: secItems.reduce((s, i) => s + i.score, 0), max: SECTION_MAX_SCORES[sec] };
              });
              return (
                <div key={ev.evaluation.id} className="card" style={{ marginBottom: 'var(--space-md)' }}>
                  <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
                      ...
                      <span className={`badge ${TRACK_BADGE_CLASS[ev.evaluation.track]}`}>{TRACK_LABELS[ev.evaluation.track]}</span>
                    </div>
                    <div className="flex gap-sm items-center">
                      <span className={`badge ${EVALUATION_STATUS_BADGE_CLASS[ev.evaluation.status]}`}>{EVALUATION_STATUS_LABELS[ev.evaluation.status]}</span>
                      <span className="text-sm" style={{ fontWeight: 700, color: ev.evaluation.passed ? 'var(--color-success)' : 'var(--color-danger)' }}>{ev.evaluation.passed ? '✅ 合格' : '❌ 不合格'}</span>
                      <span className="text-sm font-semibold">{ev.totalScore}/{OSCE_TOTAL_SCORE}点 ({ev.scoreRate}%)</span>
                    </div>
                  </div>
                  <div className="card-body">
                    <div className="text-sm text-secondary" style={{ marginBottom: 'var(--space-sm)' }}>評価者: {ev.evaluator.name}</div>
                ...
                <div className="card-body">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
                    <div style={{ display: 'flex', gap: 'var(--space-sm)', alignItems: 'center' }}>
                      <span className={`badge ${LEVEL_BADGE_CLASS[c.level.code]}`} style={{ fontSize: '0.85rem', padding: '4px 12px' }}>{c.level.name}</span>
                      {c.certification.track && <span className={`badge ${TRACK_BADGE_CLASS[c.certification.track]}`}>{TRACK_LABELS[c.certification.track]}</span>}
                      <span className={`badge ${CERTIFICATION_STATUS_BADGE_CLASS[c.certification.status]}`}>{CERTIFICATION_STATUS_LABELS[c.certification.status]}</span>
                    </div>
                    <div className="text-sm text-secondary">
                      申請日: {new Date(c.certification.appliedAt).toLocaleDateString('ja-JP')}
                      {c.certification.decidedAt && <> | 決定日: {new Date(c.certification.decidedAt).toLocaleDateString('ja-JP')}</>}
                    </div>
                  </div>
