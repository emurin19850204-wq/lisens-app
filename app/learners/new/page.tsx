...
export default function NewLearnerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await addLearner({ name: name.trim(), email: email.trim(), organizationId, tracks, hireDate: hireDate || null });
    router.push('/learners');
  };

  return (
    <div className="page-container">
      <nav className="breadcrumb"><Link href="/">ホーム</Link><span className="breadcrumb-separator">/</span><Link href="/learners">研修者一覧</Link><span className="breadcrumb-separator">/</span><span>新規登録</span></nav>
      <h1 className="page-title">👤 研修者 新規登録</h1>
      <p className="text-secondary" style={{ marginBottom: 'var(--space-lg)' }}>新しいトレーナー候補を登録します。登録後、共通コアの受講を開始できます。</p>
      <form onSubmit={handleSubmit}>
        <div className="card" style={{ marginBottom: 'var(--space-md)' }}>
          <div className="card-header">📋 基本情報</div>
          <div className="card-body">
            <div className="form-group"><label className="form-label">氏名 <span style={{ color: 'var(--color-danger)' }}>*</span></label><input type="text" className="form-input" value={name} onChange={e => setName(e.target.value)} placeholder="例: 山田 太郎" />{errors.name && <div className="text-sm" style={{ color: 'var(--color-danger)', marginTop: '4px' }}>{errors.name}</div>}</div>
            <div className="form-group"><label className="form-label">メールアドレス <span style={{ color: 'var(--color-danger)' }}>*</span></label><input type="email" className="form-input" value={email} onChange={e => setEmail(e.target.value)} placeholder="例: yamada@element.com" />{errors.email && <div className="text-sm" style={{ color: 'var(--color-danger)', marginTop: '4px' }}>{errors.email}</div>}</div>
        ...
        <div className="card" style={{ marginBottom: 'var(--space-md)' }}>
          ...
          <div className="card-body">
            <p className="text-sm text-secondary" style={{ marginBottom: 'var(--space-md)' }}>受講する専門コースを選択してください（複数選択可）。<br />※ 共通コアは全トラック共通で自動的に受講対象になります。</p>
            <div style={{ display: 'flex', gap: 'var(--space-lg)', flexWrap: 'wrap' }}>
              {(['weight', 'pilates', 'stretch'] as TrackCode[]).map(track => (
                <label key={track} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)', cursor: 'pointer', padding: 'var(--space-sm) var(--space-md)', borderRadius: 'var(--border-radius-sm)', border: tracks.includes(track) ? '2px solid var(--color-primary)' : '2px solid var(--color-border)', background: tracks.includes(track) ? 'var(--color-primary-light, #eef2ff)' : 'transparent', transition: 'all 0.15s ease' }}>
                  <input type="checkbox" checked={tracks.includes(track)} onChange={() => toggleTrack(track)} />
        ...
        <div className="card" style={{ marginBottom: 'var(--space-lg)' }}>
          ...
          <div className="card-body">
            <table><tbody>
              ...
              <tr><td className="text-secondary">所属店舗</td><td>{stores.find(s => s.id === organizationId)?.name || '—'}</td></tr>
              <tr><td className="text-secondary">受講トラック</td><td>{tracks.length > 0 ? tracks.map(t => TRACK_LABELS[t]).join(' / ') : '—'}</td></tr>
              <tr><td className="text-secondary">初期レベル</td><td><span className="badge badge-muted">Lv0 未認定</span></td></tr>
            </tbody></table>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 'var(--space-md)', justifyContent: 'flex-end' }}>
          <Link href="/learners" className="btn btn-outline">キャンセル</Link>
          <button type="submit" className="btn btn-primary" disabled={isSubmitting}>{isSubmitting ? '登録中...' : '👤 研修者を登録する'}</button>
        </div>
      </form>
    </div>
  );
